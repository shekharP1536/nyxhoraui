  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium rounded-md p-2 bg-muted/50 hover:bg-muted border border-border/50 hover:border-border transition-all duration-200"
      >
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-52 rounded-md bg-popover border border-border shadow-xl shadow-black/10 z-50 overflow-hidden animate-in fade-in-0 zoom-in-95 slide-in-from-top-2 duration-200">
          <div className="p-1">
            {/* View as Markdown */}
            <button
              onClick={handleCopyPage}
              className="flex items-center gap-3 w-full px-3 py-1 text-sm rounded-md hover:bg-muted transition-colors"
            >
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {isNew && (
            <span className="px-2 py-0.5 text-xs font-medium bg-gradient-to-r from-green-500 to-cyan-500 text-white rounded-full">
              New
            </span>
          )}
          {special && (
...
export function DocsPreview({
  ...
  return (
    <section className="space-y-4">
      ...
      <ComponentPreview preview={previewCode} code={code} />
    </section>
  );
}
    ...
    <div className="rounded-xl border border-border/50 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-1 bg-muted/30 border-b border-border/50">
        <div className="flex gap-1">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 cursor-pointer text-sm rounded-sm transition-colors ${
              activeTab === "preview"
      ...
      {activeTab === "preview" || !hasCode ? (
        ...
        <pre className="px-4 bg-background overflow-hidden">
          <code
            ref={codeRef}
            className="hljs language-tsx text-sm overflow-hidden"
          >
            {code}
          </code>
        </pre>
      ...
      <div className="relative">
        ...
        >
          <div className="flex max-h-[900px]">
            {showLineNumbers && (
              <div className="flex-shrink-0 py-4 pl-4 pr-3 select-none text-right  border-zinc-800/50">
                {lines.map((_, i) => (
                  <div
                    key={i}
                    className="text-xs leading-6 text-zinc-600 font-mono"
                  >
                    {i + 1}
                  </div>
                ))}
            ...
            <pre
              className={`flex-1  px-4 ${!showLineNumbers ? "pl-4" : "pl-3"}`}
            >
              <code
                ref={codeRef}
          ...
          <tbody>
            {props.map((prop, index) => (
              <tr
                key={prop.name}
                className={
                  index < props.length - 1 ? "border-b border-border/50" : ""
                }
              >
  ...
  return (
    <div className="relative">
      ...
      <div className="relative">
        <div
          ref={containerRef}
          className="bg-background dark:bg-zinc-950  transition-all duration-300"
          style={{
            maxHeight: isExpanded ? "none" : `${maxHeight}px`,
            overflow: isExpanded ? "auto" : "hidden",
          }}
        >
          ...
          <pre className={`flex-1  px-4 ${!showLineNumbers ? "pl-4" : "pl-3"}`}>
            <code ref={codeRef} className={`hljs text-sm leading-6 block`}>
              {code}
            </code>
          </pre>
        </div>
...
export function Credits(name: string, url: string) {
  return (
    <div className="rounded-lg border border-border p-4 bg-muted/50">
      <h2 className="text-2xl font-bold">Credits</h2>
      <p className="text-muted-foreground">
        The component is built on top of <a href={url}>{name}</a>. A big
        thank you to the developers and contributors of this project for
        providing the tools and resources to make this possible!
      </p>
    </div>
  );
}
